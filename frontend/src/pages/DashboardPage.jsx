      ...
      <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            {(kpis.total_feedback ?? 0) > 0 && (
              <span className="px-2.5 py-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-semibold rounded-lg border border-emerald-500/20">
                {kpis.total_feedback} Record{kpis.total_feedback === 1 ? '' : 's'} Plotted
              </span>
            )}
          </div>

          {(kpis.total_feedback ?? 0) === 0 ? (
            <div className="p-8 bg-white dark:bg-dark-card border border-dashed border-gray-200 dark:border-dark-border rounded-2xl shadow-sm text-center space-y-3">
              <div className="mx-auto w-12 h-12 rounded-2xl bg-brand-50 dark:bg-brand-950/40 text-brand-500 flex items-center justify-center">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 max-w-md mx-auto leading-relaxed">
                Visual charts and telemetry graphs are generated exclusively from your uploaded feedback and datasets. Ingest customer feedback or upload CSV / Excel spreadsheets to generate live charts.
              </p>
              <div className="pt-2">
                <a
                  href="/feedback"
                  className="inline-flex items-center px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white text-xs font-semibold rounded-xl shadow-md shadow-brand-500/20 transition-colors"
                >
                  <FileText className="w-3.5 h-3.5 mr-1.5" /> Upload Dataset
                </a>
              </div>
            </div>
          ) : (
